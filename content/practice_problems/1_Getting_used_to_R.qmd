...
The colon displays a sequence of numbers between two numbers
...
Displays how long a sequence between two numbers is

```{r}
max(x)
```

Displays the greatest number in a sequence

```{r}
x[x < 10]
```

Displays all numbers in the sequence that are less than 10

```{r}
x^2
```

Raises all numbers in the sequence to the second power

```{r}
x[ x < 12 & x > 7]
```

Displays all numbers in the sequence that are greater than 7 and less
than 12
...
Sequence of numbers from -1 to 2
...
Makes numbers in the parenthesis negative
...
```{r}
names(sleep)[names(sleep) %in% "BodtWt"]<- "Body_Weight"
```
...
According to the graph, as brain weight increases, total sleep decreases
