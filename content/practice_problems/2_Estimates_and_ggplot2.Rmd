...
```{r}
ggplot(sleep, aes(x=TotalSleep, y = BrainWt)) +
  geom_point()+
  xlab("Total sleep (hours per day")+ggtitle("BrainWt decreases W totalsleep")+
...
ggplot(sleep_by_primate
  ...
  geom_col(size = 3) +
  geom_errorbar(aes(ymin=TotalSleep-ci, ymax=TotalSleep+ci), size=1.5)+
  ylab("Total sleep (hours per day")+ggtitle("Total sleep changes pertaining to primate")
```

```{r}
sleep$Predation <- factor(sleep$Predation)
...
ggplot(sleep_by_predation
  ...
  ylab("Total sleep (hours per day")+ggtitle("Sleep based on predation risk")
```
...
A 95 percent confidence interval indicates that a piece of sampled data
is likely to fall in a particular interval 95 percent of the time
