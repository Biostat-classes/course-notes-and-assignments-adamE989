...
people that turn their right ear vs people that turn their left ear

An alternative hypothesis is that there is a statistically significant
difference between the two

```{r}
sampling_experiment = data.frame(sucess=rbinom(10000, 25, .5))
ggplot(sampling_experiment, aes(x=sucess)) +
  geom_histogram()
```
...
```{r}
binom.test(x=33, n=100, p=0.25, alternative = "greater")
```

p-value is less than 0.05, so we reject the null
