...
```{r}
strawberry <- c(239, 176, 235, 217, 234, 216, 318, 190, 181,225) 

z.test(x=strawberry, mu=205, sigma.x = sqrt(1500))
```
...
```{r}
strawberry <- c(239, 176, 235, 217, 234, 216, 318, 190, 181,225) 
wilcox.test(x=strawberry, mu=205 )
```

sign test

```{r}
SIGN.test(x=strawberry, mu=20)
```
...
```{r}
binom.test(25, 30, p=0.8)
```

Here, we reject the null

```{r}
binom.test(25, 30, p=0.5)
```
...
T test, as variance is not given

```{r}
Rates<- c(0.9, 1.4, 1.2, 1.2, 1.3, 2.0, 1.4, 1.6)

t.test(x=Rates, mu=1.4)
```
